...
export const getMsItemIdConverter = (items: Item[]) => {
    ...
    const itemIdToMsItemId = Object.fromEntries(items.map(item => ([item.id, itemToMsItemId(item)])))
    const msItemIdToItemId = Object.fromEntries(items.map(item => ([itemToMsItemId(item), item.id])))
