...
export default function Result({
  ...
  questInfo: {
    [key: string]: { area: string; name: string; id: string; ap: string }
  }
  ...
  const itemToQuery = queryQueries
    .split(',')
    .reduce((acc: { [key: string]: number }, cur: string) => {
  ...
  const totalAp =
    typeof query.total_ap == 'string'
      ? parseInt(query.total_ap)
      : questLaps
          .map(({ id, lap }) => lap * parseInt(questInfo[id].ap))
          .reduce((acc, cur) => acc + cur)

  const questIds = questLaps.map(({ id }) => id)
  questToDrops = Object.fromEntries(
    Object.entries(questToDrops).filter(([questId, drops]) =>
      questIds.includes(questId)
    )
  )
    ...
    <>
      ...
      <section>
        ...
        <QuestTable
          ...
          itemIndexes={
            itemInfo as { [key: string]: { id: string; name: string } }
          }
        />
      </section>
      <section>
          ...
          <section>
            <SumTable
              rows={[
                ...
                {
                  key: '費用',
                  value: (totalAp / 144 / 168).toFixed(1),
                  unit: '万円',
                },
              ]}
            />
