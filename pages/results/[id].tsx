...
export default function Result({
    ...
    if (!params || !quests || !items || !drop_rates) {
        return (
            <>
                <h1>結果が見つかりませんでした</h1>
    ...
    const paramItemIndexes = Object.fromEntries(paramItems.map(({id, count}) => ([id, count])))
            ...
            <section>
                ...
                {Object.entries(largeItemGroups).map(([largeCategory, itemGroups]) => (
                    <details className="item-details" key={largeCategory}>
                        ...
                        <ItemTable itemGroups={itemGroups} itemToQuery={paramItemIndexes}/>
                    </details>
                ))}
            </section>
            <section>
