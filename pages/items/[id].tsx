...
export default function Item(
  ...
  dropRates.sort((a, b) =>
    a.item_id == id
      ...
      : b.item_id == id
      ? 1
      : parseInt(a.item_id, 36) - parseInt(b.item_id, 36)
  )
  const dropGroups = _.groupBy(dropRates, ({ quest_id }) => quest_id)
  const idToDropRate = (quest_id: string, item_id: string) =>
    dropGroups[quest_id].find((row) => row.item_id == item_id) || {
      [dropRateKey]: 0,
    }
  const getDropRate = (quest_id: string, item_id: string, key: DropRateKey) =>
    idToDropRate(quest_id, item_id)[dropRateKey] || 0
  const selectedQuests =
    dropRateStyle == 'rate'
      ? quests
          ...
          .sort(
            (a, b) =>
              getDropRate(b.id, id, dropRateKey) -
              getDropRate(a.id, id, dropRateKey)
          )
      : quests
          .slice()
          .sort(
            (a, b) =>
              a.ap / getDropRate(a.id, id, dropRateKey) -
              b.ap / getDropRate(b.id, id, dropRateKey)
          )
    ...
    <>
      ...
      <form className="select">
        <div>
          <input
            type="radio"
            name="drop-rate-key"
            checked={dropRateKey == 'drop_rate_1'}
            onChange={() => {
              setDropRateKey('drop_rate_1')
            }}
            id="drop-rate-1"
          />
        ...
        <div>
          <input
            type="radio"
            name="drop-rate-key"
            checked={dropRateKey == 'drop_rate_2'}
            onChange={() => {
              setDropRateKey('drop_rate_2')
            }}
            id="drop-rate-2"
          />
      ...
      <form className="select">
        <div>
          <input
            type="radio"
            name="drop-rate-style"
            checked={dropRateStyle == 'ap'}
            onChange={() => {
              setDropRateStyle('ap')
            }}
            id="style-ap"
        ...
        <div>
          <input
            type="radio"
            name="drop-rate-style"
            checked={dropRateStyle == 'rate'}
            onChange={() => {
              setDropRateStyle('rate')
            }}
            id="style-rate"
          />
