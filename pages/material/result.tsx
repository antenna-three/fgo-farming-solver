...
const Result = ({ items }: { items: (Item & { category: string })[] }) => {
  const router = useRouter()
  const filtered = Object.fromEntries(
    Object.entries(router.query).filter(([k, v]) => typeof v == 'string')
  ) as { [key: string]: string }
  const amounts = Object.fromEntries(
    Object.entries(filtered).map(([k, v]) => [k, parseInt(v)])
  )
  ...
  const initialTargetCategories = Object.fromEntries(
    categories.map((category) => [category, category == '強化素材'])
  )
  const [targetCategories, setTargetCategories] = useState(
    initialTargetCategories
  )
      ...
      const queryItems = filteredItems
        .filter(
          (item) =>
            ...
            getSolverId(item) &&
            targetCategories[getLargeCategory(item.category)]
        )
    ...
    <>
      ...
      <form onSubmit={goSolver}>
        <input
          type="checkbox"
          id="hide-sufficient"
          checked={hideSufficient}
          onChange={onChangeSufficient}
        />
                ...
                <tbody>
                  {subItemGroup.map(([category, subItems]) => (
                    <Fragment key={category}>
                      ...
                      {subItems
                        .filter(
                          (item) => !hideSufficient || deficiencies[item.id] > 0
                        )
                        .map((item) => (
                          <tr key={item.id}>
          ...
          {categories.map((category) => (
            <div key={category}>
              <input
                ...
                id={category}
                name={category}
                checked={targetCategories[category]}
                onChange={handleChangeTargetCategories}
              />
