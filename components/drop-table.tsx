...
export default function DropTable({
    ...
    const colSpan = Object.values(dropGroups).reduce((acc, cur) => (cur.length > acc ? cur.length : acc), 0) * 2
    return (
        <table>
            <thead>
                <tr>
                    <th>エリア</th>
                    <th>クエスト</th>
                    <th colSpan={colSpan}>ドロップ ({dropRateStyle == 'rate' ? '%' : 'AP/個'})</th>
                </tr>
            </thead>
            <tbody>
                {quests.map(quest => (
